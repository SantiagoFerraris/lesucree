                  ...
                  try {
                    ...
                    if (variantName && variantPriceStr) {
                                ...
                                const matchingVariant = variants?.find(
                                  ...
                                  if (matchingVariant) {
                                                  const { error } = await admin.from('product_variants').update({ price: vPrice, label: variantName }).eq('id', matchingVariant.id);
                                                  if (error) { errors.push(`Row ${i + 1}: ${error.message}`); } else { updated++; updatedProductIds.add(product.id); }
                                  } else {
                                                  // Check if product has any variants at all in the DB
                                    const productHasVariants = variants?.some(v => v.product_id === product.id);
                                                  if (!productHasVariants) {
                                                                    // Product has no variants in DB — update base price from the "price" column
                                                    const basePrice = parseFloat(priceStr);
                                                                    if (!isNaN(basePrice)) {
                                                                                        const { error: baseError } = await admin.from('products').update({ price: basePrice }).eq('id', product.id);
                                                                                        if (baseError) {
                                                                                                              errors.push(`Row ${i + 1}: ${baseError.message}`);
                                                                                            } else {
                                                                                                              updated++;
