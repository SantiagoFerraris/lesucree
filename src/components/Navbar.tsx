...
export default function Navbar() {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 h-[72px] md:h-[72px] flex items-center bg-cream/95 backdrop-blur-[10px] transition-shadow duration-300 ${scrolled ? 'shadow-sm' : ''}`}>
      ...
      {open && (
        <div
          className="fixed left-0 right-0 bottom-0 z-50 flex flex-col items-center justify-center gap-8 animate-fade-in"
          style={{ top: '72px', backgroundColor: 'hsl(30, 100%, 97%)' }}
        >
          {links.map(l => (
            <Link
              key={l.to}
