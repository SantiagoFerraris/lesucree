...
export default function ProductDetailModal({ product, variants, onClose }: { product: Tables<'products'>; variants: Variant[]; onClose: () => void }) {
  ...
  return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-espresso/40 backdrop-blur-sm animate-fade-in p-6 sm:p-4" onClick={onClose}>
                <div className="bg-soft-white rounded-xl sm:rounded-2xl shadow-xl max-w-lg w-[calc(100vw-1rem)] sm:w-full overflow-hidden max-h-[85vh] sm:max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="p-4 sm:p-6">
                                  ...
                                  <h3 className="font-display text-xl sm:text-2xl font-bold text-espresso mt-1">{product.name}</h3>h3>
