...
export default function PagosPedidoAdmin({ order, open, onOpenChange }: Props) {
  ...
  const buildMessage = (kind: MessageKind): string => {
    ...
    if (kind === 'solicitar_sena') {
      return `¡Hola ${customer}!\n\nPara confirmar tu pedido, te pedimos una seña del 50% del total.\n\n\u{1F4B0} Seña a abonar: ${formatPrice(depositSuggested)}\n\u{1F4B0} Saldo pendiente: ${formatPrice(total - depositSuggested)}\n\u{1F4B0} Total del pedido: ${formatPrice(total)}\n\n\u{1F4CB} Tu pedido:\n${orderDetails}\n\n\u{1F4B3} Podés abonar por transferencia a cualquiera de los siguientes alias:\nAlias: ${cfg.alias}${cfg.cbu ? `\nCBU: ${cfg.cbu}` : ''}\n\nCuando confirmes el pago, nos avisás y aseguramos tu pedido. El saldo restante lo abonás el día del retiro.\n\n\u{1F550} Horarios: ${cfg.hours}\n\n¿Preguntas? Escribinos 📱`;
    }
    if (kind === 'confirmar_sena') {
      return `¡Hola ${customer}! \u{1F49A}\n\nConfirmamos que recibimos tu seña correctamente, ¡gracias!\nTu pedido ya quedó reservado \u{1F64C}\n\n\u{1F4CD} Dirección de retiro:\n${cfg.pickupAddress}\n\n\u{1F4C5} ${pickupDate} — ${pickupTime}\n\n\u{1F4B0} RESUMEN DEL PAGO:\n━━━━━━━━━━━━━━━━━━━━━━━\n- Seña recibida: ${formatPrice(calc.amountPaid)}\n- Saldo pendiente: ${formatPrice(calc.remainingBalance)}\n- Total del pedido: ${formatPrice(total)}\n\n\u{1F4CB} Tu pedido:\n${orderDetails}\n\n\u{1F4B3} El saldo lo abonás el día del retiro\n\n¡Te esperamos! \u{1F370}`;
    }
    // pedido_listo
    return `¡Hola ${customer}!\n\nTu pedido está listo para retirar \u{1F389}\n\n\u{1F4CB} Pedido: #${String(order.id).slice(0, 8).toUpperCase()}\n\u{1F4C5} Fecha: ${pickupDate}\n\u{1F550} Horario: ${pickupTime}\n\u{1F4CD} Dirección: ${cfg.pickupAddress}\n\n\u{1F4B0} Total: ${formatPrice(total)}\n(Seña pagada: ${formatPrice(calc.amountPaid)} | Saldo a abonar: ${formatPrice(calc.remainingBalance)})\n\nGracias por confiar en Le Sucrée Pastelería. Esperamos verte pronto \u{1F49A}`;
  };
