...
export default function Contacto() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (isHoneypotFilled(honeypot)) { toast.success('¡Mensaje enviado! Te responderemos pronto.'); return; }
    if (isSubmissionTooFast(formLoadedAt.current)) { toast.error('Por favor esperá un momento antes de enviar.'); return; }
    if (!checkRateLimit('contact')) { toast.error('Demasiados envíos. Por favor esperá un momento.'); return; }

    const name = form.name.trim();
    const email = form.email.trim();
    const message = form.message.trim();

    if (!name || !email || !message) { toast.error('Por favor completá todos los campos obligatorios'); return; }
    if (!EMAIL_REGEX.test(email)) { toast.error('Ingresá un email válido'); return; }
    if (message.length > MAX_MESSAGE_LENGTH) { toast.error(`El mensaje no puede superar los ${MAX_MESSAGE_LENGTH} caracteres`); return; }
    ...
    if (error || !result?.success) {
      ...
      try {
        await supabase.functions.invoke('send-contact-notification', { body: { name, email, message } });
      } catch { /* non-blocking */ }
      ...
      <div className="py-10 sm:py-16 md:py-20 px-3 sm:px-4">
        <div className="container">
            ...
            <div className="space-y-6">
              <div>
                ...
                <div className="mt-4 space-y-3">
                  <a href={WHATSAPP_URL} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 text-espresso/70 hover:text-dusty-pink transition-colors">
                    <span className="text-lg">📱</span> WhatsApp
                  </a>
                  <a href={INSTAGRAM_URL} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 text-espresso/70 hover:text-dusty-pink transition-colors" aria-label="Instagram de Le Sucrée">
                    <Instagram size={18} /> {INSTAGRAM_HANDLE}
                  </a>
                  <a href={`tel:+${WHATSAPP_NUMBER}`} className="flex items-center gap-3 text-espresso/70 hover:text-dusty-pink transition-colors">
          ...
          <div className="text-center mt-16">
            <a href={WHATSAPP_URL} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 rounded-full bg-[#25D366] text-white px-8 py-3.5 text-[15px] font-semibold uppercase tracking-[0.1em] hover:bg-[#1da851] hover:scale-[1.02] transition-all duration-300 active:scale-95">
