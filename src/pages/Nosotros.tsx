...
export default function Nosotros() {
  ...
  return (
    <section className="pt-[72px]">
      {/* Hero */}
      <div className="bg-blush py-20 md:py-24 px-4">
        <div className="container text-center">
          <h1 className="font-display text-[32px] md:text-[44px] font-bold text-espresso">Nuestra Historia</h1>
          <SectionDivider />
        </div>
      </div>

      {/* Story */}
      <div ref={storyReveal.ref} className="py-16 md:py-20 px-4">
        <div className={`container max-w-[680px] ${storyReveal.isVisible ? 'animate-fade-in-up' : 'opacity-0'}`}>
          <p className="text-[17px] leading-[1.8] text-espresso">
          ...
          <p className="text-[17px] leading-[1.8] text-espresso mt-6">
      ...
      {/* Values */}
      <div ref={valuesReveal.ref} className="py-16 md:py-20 px-4 bg-blush/30">
        <div className="container">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {values.map((v, i) => (
              <div
                key={v.title}
                className={`bg-soft-white rounded-2xl p-8 text-center shadow-sm ${valuesReveal.isVisible ? 'animate-fade-in-up' : 'opacity-0'}`}
